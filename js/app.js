const JSON_URL = 'https://raw.githubusercontent.com/yasseralsebaee2/APFC-Titania/refs/heads/main/_titania-map.json_';
    ...
    function metricUnit(metric) {
      return metric === 'piles' ? 'piles' : metric === 'lm' ? 'Lm' : 'mÂ³';
    }
    ...
    function periodTitle(dateKey) {
      ...
      if (chartGranularity === 'week') return getCW(dateKey) + ' Â· ' + formatDateLabel(dateKey);
    ...
    function aggregateDailyMetrics(rows, metric) {
      ...
      const lastKey = periodKey(previousDayKey(), chartGranularity);
    ...
    function showProductionTooltip(evt, dataPoint, valueFormatter, options = {}) {
      ...
      if (options.groupBy === 'day' && Array.isArray(dataPoint.items) && dataPoint.items.length) {
        ...
        dataPoint.items.forEach(item => {
          const metaBits = [];
          if (item.id) metaBits.push(item.id);
          if (item.date) metaBits.push(formatDateLabel(item.date));
          html += `<div class="tooltip-list-item"><span>${metaBits.join(' Â· ') || item.id}</span><span>${valueFormatter(item.value)}</span></div>`;
        });
        html += `</div>`;
      }
...
function renderProductionMetricChart(project, key, forceAnimate = false) {
    ...
    function renderDashboard(project) {
      ...
      els.kpiEtaDays.textContent = stats.etaMonths !== null ? `${stats.etaMonths.toFixed(1)} mo` : 'â€”';
      renderChart(rows);
    ...
    function formatDateTimeLabel(value) {
      if (!(value instanceof Date) || Number.isNaN(value.getTime())) return 'â€”';
    ...
    function renderTimelineTable(rows) {
      ...
      els.timelineTableBody.innerHTML = rows.map(item => `
        <tr>
          ...
          <td>${Number.isFinite(item.gap1) ? `${formatNumberOneDecimal(item.gap1)} hr` : 'â€”'}</td>
          <td>${Number.isFinite(item.gap2) ? `${formatNumberOneDecimal(item.gap2)} hr` : 'â€”'}</td>
    ...
    function renderTimelineSummary(rows, forceAnimate = false) {
      ...
      const label = timelineState.start || timelineState.end
        ? `${timelineState.start || 'â€¦'} â†’ ${timelineState.end || 'â€¦'}`
        : 'Total / All Dates';
    ...
    function renderTimelinePage(project, forceAnimate = false) {
      ...
      rows.forEach(pile => {
        pile.activities.forEach(activity => {
          ...
          const html = `<div class="tooltip-title">${pile.id} Â· ${activity.label}</div><div class="tooltip-row"><span>Start</span><strong>${formatDateTimeLabel(activity.start)}</strong></div><div class="tooltip-row"><span>End</span><strong>${formatDateTimeLabel(activity.end)}</strong></div><div class="tooltip-row"><span>Duration</span><strong>${formatNumberOneDecimal(duration)} hr</strong></div>`;
